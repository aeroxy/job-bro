...
- **Cloud fields** (`providerMode === 'api'`, i.e. `'openai'` or `'anthropic'`): **API Format** toggle, base_url, model, api_key (password show/hide), custom_headers (JSON textarea), timeout, stream_timeout, **concurrency**, max_tokens, temperature; plus Allow Tool Calls / Structured Output / Stream Mode switches.
