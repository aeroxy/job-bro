...
- **Anthropic differences** (`isAnthropic`): base_url/model placeholders become `https://api.anthropic.com/v1` / `claude-opus-5`; the Structured Output blurb names `output_config.format`; the Stream Mode switch is hidden (that path always streams) and the timeout hints flip so "Stream Timeout" is the live one. A note under the toggle explains the `x-api-key` + `x-claude-code-session-id` headers and links claude-proxy. The session claim is scoped to what actually happens: the id is stable **per job** (analysis and resume share it; chat seeds separately), which keeps a proxy's cache readable across a run's turns and across re-runs — it does not create hits between modes, whose prompt prefixes differ. See [`lib.md` → `llm-handlers.ts`](lib.md#llm-handlersts).
