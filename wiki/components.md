...
- **Qwen-only fields** (only when backend is `'qwen-chat'`): Auth Status (token check), Device Identity (device ID + rotating `ssxmod_itna` fingerprint, Update button), **Model** picker (`config.qwenModel`, one of `QWEN_MODELS`: `qwen3.8-max-preview` / `qwen3.7-max` / `qwen3.7-plus`, normalized via `normalizeQwenModel` with fallback to `QWEN_MODELS[0]`), and **Concurrency** — caps how many evaluators hit `chat.qwen.ai` in parallel (`config.concurrency ?? 2`); lowering to 1 mitigates the anti-bot "overloaded" throttle.
